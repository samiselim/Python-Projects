...
master: mavserial = mavutil.mavlink_connection("/dev/ttyAMA0", baud=57600)
print(master)
...
def get_location(relative_alt=False):
...
while True:
    if master.motors_armed() != 0 and not Arm_Flag and get_location().lat != 0:
        Arm_Flag = True
        home = get_location()
        print(f"armed and home location is : lat --> {home.lat}  long --> {home.lng}")
    elif master.motors_armed() == 0 and Arm_Flag == True:
        Arm_Flag = False
        home = None
    ...
    if home is not None:
        ...
        if distance(home.lat, home.lng, cl.lat, cl.lng) > 0.02 and not Loiter_Flag:
